...
def crop_watermark(gradx, grady, threshold=0.4, boundary_size=2):
    ...
    # FIX: AttributeError: 'float' object has no attribute 'sqrt'
...
def watermark_detector(img, gx, gy, thresh_low=200, thresh_high=220, printval=False):
    ...
    im = img.copy()
    # FIX: TypeError: function takes exactly 4 arguments (2 given
    cv2.rectangle(im, (int(y), int(x)), (int(y+rect[1]), int(x+rect[0])), (255, 0, 0), 1)
